...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser, deliveries }) => {
  ...
  const handleApprove = (userId: string) => {
    // Aprovação direta para agilidade
    try {
  ...
  const filteredUsers = users.filter(u => filter === 'PENDING' ? !u.approved : u.approved);

  // Filtra as entregas do usuário selecionado para o Modal
  const getUserDeliveries = (user: User) => {
    if (user.role === UserRole.RESTAURANT) {
      return deliveries.filter(d => d.restaurantId === user.id);
    }
    return deliveries.filter(d => d.courierId === user.id && d.status === DeliveryStatus.DELIVERED);
  };
                        ...
                        <div key={d.id} className="border border-slate-100 rounded-xl p-3 flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-xs font-bold text-slate-800 truncate max-w-[150px]">
                              {selectedUser.role === UserRole.RESTAURANT ? d.deliveryAddress : d.restaurantName}
                            </p>
                          </div>
